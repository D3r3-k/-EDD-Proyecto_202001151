...
void UserWindow::on_guardarModificacionButton_clicked()
{
    ...
    {
        QString correo = QString::fromStdString(usuario_logeado->correo);
        if (Func::ModificarUsuario(nombres, apellidos, fecha, correo, contrasena, usuario_logeado->rol)) {
            ...
            QMessageBox::warning(nullptr, "Modificar Datos", "Hubo un error al modificar el usuario.");
        }
...
void UserWindow::on_pushButton_reporte_grafo_amistades_clicked()
{
    // Graficar amistades
    std::string path_friends = relaciones_amistad.graficarGrafoSugeridos(usuario_logeado->correo);
    QString ruta_imagen_friends = QString::fromStdString(path_friends);
    QPixmap imagen(ruta_imagen_friends);
    QPixmap imagenEscalada = imagen.scaled(ui->label_grafo_sugerencias->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation);
    ui->label_grafo_sugerencias->setPixmap(imagenEscalada);
    path_grafo = path_friends;
}


void UserWindow::on_pushButton_reporte_abrir_grafo_clicked()
{
    // Verificar que la ruta de la imagen no esté vacía
    if (path_grafo.empty()) {
        QMessageBox::warning(this, "Error", "No se ha generado ninguna imagen.");
    ...
    QString rutaImagen = QString::fromStdString(path_grafo);
    if (!QDesktopServices::openUrl(QUrl::fromLocalFile(rutaImagen))) {
        QMessageBox::warning(this, "Error", "No se pudo abrir la imagen.");
    }
}
