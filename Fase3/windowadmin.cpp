...
void AdminWindow::on_pushButton_reportes_generar_clicked()
{
    ...
    // Graficar amistades
    std::string path_friends = relaciones_amistad.graficarGrafo();
    ...
    // Graficar amistades
    std::string path_ady = relaciones_amistad.tablaAdyacencia();
    QString ruta_imagen_ady = QString::fromStdString(path_ady);
    QPixmap imagen4(ruta_imagen_ady);
    QPixmap imagenEscalada4 = imagen4.scaled(ui->img_amistades_ady->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation);
    ui->img_amistades_ady->setPixmap(imagenEscalada4);
    img_tabla_ady = path_ady;
...
{
    // Verificar que la ruta de la imagen no esté vacía
    if (img_tabla_ady.empty()) {
        QMessageBox::warning(this, "Error", "No se ha generado ninguna imagen.");
        return;
    }
    QString rutaImagen = QString::fromStdString(img_tabla_ady);
    if (!QDesktopServices::openUrl(QUrl::fromLocalFile(rutaImagen))) {
        QMessageBox::warning(this, "Error", "No se pudo abrir la imagen.");
    }
}
